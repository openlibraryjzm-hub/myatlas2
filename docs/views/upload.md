...
- **Target Sub-Atlas Archive Destination Selector**: Explicit header panel allowing users to select or switch the destination sub-atlas (e.g. `myatlas`, `gamesatlas`, `amberatlas`) for the upload batch. Ingested items are tagged with `atlas_id: selectedAtlasSlug`.
