...
- **Top Header Integration**: Accessible directly from the top navigation bar ([`Navbar.jsx`](../../src/components/Navbar.jsx)) via the `<Layers size={16} />` icon button ("Bulk Tag Injector").
...
- Clicking **`Inject Tag to X Items`** invokes `bulkInjectTagToFilter(targetFilterTag, newTagToInject)` in [`localDb.js`](../../src/services/localDb.js).
